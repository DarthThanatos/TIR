...
class LedResources(resource.CoAPResource):
    def __init__(self):
        resource.CoAPResource.__init__(self)
        self.visible = True
    ...
    def render_PUT(self, request):
        ...
        if request.payload == '0':
            payload = "Led wrote: " + str(32 + 0)
            try:
                self.ser.write(chr(32))
            except Exception:
                pass
            print payload
        elif request.payload == '1':
            payload = "Led wrote: " + str(32 + 1)
            try: 
...
class ButtonResources(resource.CoAPResource):
    def __init__(self):
        resource.CoAPResource.__init__(self)
        self.visible = True
        self.observable = True
        self.state = "0"
        thread.start_new_thread( thread_func, (self,) )
        self.notify()



    def notify(self):
        log.msg('ButtonResources: trying to send notifications')
        #self.state = str (-int(self.state))
        self.updatedState()
...
led = LedResources()
dev.putChild('led', led)

button = ButtonResources()
dev.putChild('button', button)
